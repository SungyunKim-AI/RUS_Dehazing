...
for epoch in range(opt.niter):
  ...
  for i, data in enumerate(dataloader, 0):
    ...
    if ganIterations % opt.display == 0:
      print(f'[{epoch}/{opt.niter}][{i}/{len(dataloader)}] L_D: {L_tran_.data[0]} L_img: {L_tran_.data[0]} L_G: {L_img.data[0]} D(x): {L_img.data[0]} D(G(z)): {L_img.data[0]} / {L_img.data[0]}')
      sys.stdout.flush()
      trainLogger.write(f'{i}\t{L_img.data[0]}\t{L_img.data[0]}\t{L_img.data[0]}\t{L_img.data[0]}\t{L_img.data[0]}\t{L_img.data[0]}\n')
      trainLogger.flush()
