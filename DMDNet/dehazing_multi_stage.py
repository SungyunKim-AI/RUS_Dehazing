...
import numpy as np
import csv
from torch.nn.functional import l1_loss, threshold
...
def test_stop_before_zero(model, test_loader, device, show=False, write_csv=False):
    entropy_module = Entropy_Module()
    stage = 300
    model.eval()
    psnr_sum, ssim_sum = 0.0, 0.0
...
def test_stop_when_threshold(model, test_loader, device, show=False, write_csv=False):
    entropy_module = Entropy_Module()
    ...
    for batch in tqdm(test_loader):
        ...
        last_etp = entropy_module.get_entropy((init_hazy*255).astype(np.uint8))
        
        #last_niqe = m.niqe(cv2.cvtColor(init_hazy,cv2.COLOR_BGR2GRAY))
        #print(last_niqe)
        ...
        for i in range(1,stage+1):
            ...
            hazy = (hazy_images[0] * 0.5 + 0.5).detach().cpu().numpy().transpose(1,2,0)
            #airlight= (airlight_module.getAirlight((hazy*255).astype(np.uint8))/255).astype(np.float32)
            ...
            prediction = (hazy-airlight)/(trans+e) + airlight
            prediction = np.clip(prediction,0,1)
            hazy_images = torch.Tensor(((prediction-0.5)/0.5).transpose(2,0,1)).unsqueeze(0)
            ...
            if (diff_etp<thresh_hold or i==stage) and finding_max:
                step = i-1
                psnr_sum +=_psnr
                ssim_sum +=_ssim
                print(f'last_stage = {step}')
        ...
        init_clear_img = cv2.cvtColor(init_clear,cv2.COLOR_RGB2BGR)
        init_airlight_img = cv2.cvtColor(init_airlight,cv2.COLOR_RGB2BGR)
        airlight_img = cv2.cvtColor(airlight,cv2.COLOR_RGB2BGR)
        one_shot_prediction = cv2.cvtColor(one_shot_prediction.astype(np.float32),cv2.COLOR_BGR2RGB)
        
        if show:
            cv2.imshow('final depth', depth/10)
            cv2.imshow('last stage prediction',prediction)
            cv2.imshow("init_hazy",init_hazy_img)
            cv2.imshow("init_clear",init_clear_img)
            cv2.imshow("init_airlight",init_airlight_img)
            cv2.imshow("final_airlight",airlight_img)
            cv2.imshow("init_depth", init_depth/10)
            cv2.imshow("init_clear_depth", init_clear_depth/10)
            cv2.imshow('one_shot_prediction',one_shot_prediction)
            cv2.waitKey(0)
...
if __name__ == '__main__':
    ...
    dataset_test = Dense_Haze_Dataset('D:/data/Dense_Haze/train',[net_w,net_h],printName=True,returnName=True)
    ...
    test_stop_when_threshold(model,loader_test,device,show=True,write_csv=False)